    <footer class="page-footer blue">
      <div class="container">
        <div class="row">
          <div class="col l6 s12">
            <h5 class="white-text">MD. Amir Hossain</h5>
            <ul>
              <li><a class="grey-text text-lighten-3" href="https://github.com/amirphp7/bongo-frontend-test">Github</a></li>
              <li><a class="grey-text text-lighten-3" href="https://www.linkedin.com/in/amirphp7/">LinkedIn</a></li>
            </ul>
          </div>
          <div class="col l4 offset-l2 s12">
            <h5 class="white-text">Interview</h5>
            <ul>
              <li><a class="grey-text text-lighten-3" href="anagram.php">Ques 1</a></li>
              <li><a class="grey-text text-lighten-3" href="designPattern.php">Ques 2</a></li>
              <li><a class="grey-text text-lighten-3" href="videoPlayer.php">Ques 3</a></li>
            </ul>
          </div>
        </div>
      </div>
      <div class="footer-copyright">
        <div class="container">
        © 2019 Bongo Built & Managed with Love by Amir Hossain.
        <a class="grey-text text-lighten-4 right" href="#!">Sitemap</a>
        </div>
      </div>
    </footer>

    <!-- Compiled and minified JavaScript -->
    <script type="text/javascript" src="https://cdnjs.cloudflare.com/ajax/libs/materialize/1.0.0/js/materialize.min.js"></script>
    <script type="text/javascript" src="assets/js/jquery-3.4.0.min.js"></script>
    <script src="http://cdnjs.cloudflare.com/ajax/libs/highlight.js/8.6/highlight.min.js"></script>
    <script src="https://www.youtube.com/iframe_api"></script>
    <script type="text/javascript" src="assets/js/main.js"></script>
    <script type="text/javascript" src="assets/js/video.js"></script>
  </body>
</html>
